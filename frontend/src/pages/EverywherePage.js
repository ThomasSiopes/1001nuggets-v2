...
function Everywhere () {
    ...
    if(!everywhereRealId || everywhereRealId === null || everywhereRealId === "undefined") return redirect(`/everywheres`);
    ...
    for(let i = 0; i < everywhere.quotes.length; ++i) {
        indexList.push(i);
    }
                            ...
                            <Col xs={12} md={6} lg={4}>
                                   ...
                                   <Row>
                                       {list1.map((index) => (
                                           <Col xs={12} className="mb-3" key={everywhere.quotes[index].quoteText}>
                                               <QuoteCard quotes={everywhere.quotes} quoteIndex={index} indexOrder={list1.concat(list3a.concat(list3b.concat(list2)))}/>
                                           </Col>
                                       ))}
                                   </Row>
                               }
                               {
                                   ...
                                   <Row className="d-xs-block d-lg-none">
                                       {list3a.map((index) => (
                                           <Col xs={12} className="mb-3" key={"false" + everywhere.quotes[index].quoteText}>
                                               <QuoteCard quotes={everywhere.quotes} quoteIndex={index} indexOrder={list1.concat(list3a.concat(list3b.concat(list2)))}/>
                                           </Col>
                                       ))}
                                   </Row>
